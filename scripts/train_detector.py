"""Training entrypoint (stage 0 placeholder)."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train detector model (placeholder).")
    parser.add_argument(
        "--config",
        type=str,
...
def main() -> int:
    ...
    if args.dry_run:
        logger.info("Dry-run finished. Training logic is not implemented in phase 0.")
        return 0

    logger.warning("Phase 0 placeholder only. Detector training will be added in phase 1.")
    return 0
